...
## Focus: RV8GR student build + RV8-R FullHW investigation
...
1. Added RV8GR student build guardrails:
...
2. Cleaned RV8GR generated artifacts:
...
4. Hardened Programmer/RV8GR integration:
   ...
   - added RV8GR `program/verify --base 0x0000` CLI aliases
...
5. Verified integration:
   ...
   - RV8GR wiring verifier: all wiring verified
   - RV8GR assembler tests: 11 pass
...
- 33 logic chips + ROM + RAM = 36 packages
...
- Dual mode, use exactly one at a time:
  ...
  - RV8-Bus in-system: leave ZIF empty, connect Programmer to RV8GR bus, hold CPU reset during flash
...
- `rv8flash.py` accepts both legacy flags and RV8GR aliases: `program FILE --base 0x0000`, `verify FILE --base 0x0000`
...
### Next steps:
- Build RV8GR physically from `RV8GR/doc/labs/README.md`
- Use `RV8GR/doc/06_debug_plan.md` probe map during bring-up
- Create KiCad schematics from `RV8GR/doc/10_kicad_modules.md` (6 sheets)
...
- The new generator must emit the safe default for unused states and reject illegal IBUS owner combinations.
