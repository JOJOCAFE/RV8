...
Wiring:
  SEL (pin 1) ← /ADDR_MODE (1=PC fetch, 0=IRL/DP data address)
  /E (pin 15) ← GND
  A inputs ← IRL[7:0]
  B inputs ← PC[7:0]
...
TEST_VECTORS = [
    (1, 0x00, 0xFF, 0x00),   # /ADDR_MODE=1 → PC
    (1, 0x42, 0xFF, 0x42),   # /ADDR_MODE=1 → PC
    (0, 0x42, 0x10, 0x10),   # /ADDR_MODE=0 → IRL
    (0, 0x00, 0xAB, 0xAB),   # /ADDR_MODE=0 → IRL
    (1, 0x80, 0x00, 0x80),   # back to PC
]
