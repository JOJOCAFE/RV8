...
module tb_rv8gr_tasks;
    ...
    initial begin
        ...
        if (cpu.pc !== 16'h8000) begin $display("FAIL: PC=%04X", cpu.pc); errors++; end else $display("PASS: PC after reset = $8000");
        if (cpu.page_reg !== 8'h80) begin $display("FAIL: PG=%02X", cpu.page_reg); errors++; end else $display("PASS: page register after reset = $80");
