...
```
ที่อยู่ $8000: $11    (LI ค่า $11)
ที่อยู่ $8001: $11    (operand = $11)
ที่อยู่ $8002: $22    
ที่อยู่ $8003: $33    
ที่อยู่ $8004: $44    
ที่อยู่ $8005: $55    
...
Flash ROM ด้วย:
```bash
python3 rv8flash.py program test_bytes.bin --base 0x8000
```
...
ROM ไฟเลี้ยง:
  ...
  pin 20 (/CE) ← /A15 (ถ้ายังไม่มี inverter ให้ต่อ GND = enable ตลอด)
  100nF คร่อม VCC-GND
...
LED (ดู IBUS จากฝั่ง A):
  pin 2 (A1=IB0) → 330Ω → LED → GND
  pin 3 (A2=IB1) → 330Ω → LED → GND
  pin 4 (A3=IB2) → 330Ω → LED → GND
  pin 5 (A4=IB3) → 330Ω → LED → GND
  pin 6 (A5=IB4) → 330Ω → LED → GND
  pin 7 (A6=IB5) → 330Ω → LED → GND
  pin 8 (A7=IB6) → 330Ω → LED → GND
  pin 9 (A8=IB7) → 330Ω → LED → GND
```

**สำคัญ**: PC reset = $8000 (bit15=1) ดังนั้น A15=1 → /A15=0 → ROM /CE=0 (enabled) ✅
...
1. Reset PC → $8000 (counter จาก Lab 03 ต้อง reset เป็น $8000)
   - ถ้า PC reset เป็น $0000: ต่อ U3 /CLR แบบ preset A15=1 (หรือต่อ A15 = VCC ชั่วคราว)
   - วิธีง่าย: ต่อ A15 = VCC เพื่อ force ให้ ROM enabled
2. ตั้ง ADDR_MODE = 0 (ให้ PC ขับ address)
3. กด Clock ดู LED:

### ตาราง LED (IB7...IB0 = data จาก ROM)

| กด Clock | Address | ROM data | IB7 | IB6 | IB5 | IB4 | IB3 | IB2 | IB1 | IB0 | ค่า | ถูก? |
|:--------:|:-------:|:--------:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:----:|
| Reset | $8000 | $11 | ○ | ○ | ○ | ● | ○ | ○ | ○ | ● | $11 | ☐ |
| 1 | $8001 | $11 | ○ | ○ | ○ | ● | ○ | ○ | ○ | ● | $11 | ☐ |
| 2 | $8002 | $22 | ○ | ○ | ● | ○ | ○ | ○ | ● | ○ | $22 | ☐ |
...
> ```
> A15=1 ($8000-$FFFF): ROM เปิด, RAM ปิด
> A15=0 ($0000-$7FFF): RAM เปิด, ROM ปิด
> ```
