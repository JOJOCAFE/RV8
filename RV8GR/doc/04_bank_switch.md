...
> แต่ ROM /WE ไม่ได้ต่อ → SB writes go nowhere (data lost silently)
...
| IRQ vector | Always bank 0 | $FF00 is in RAM — unaffected by ROM banking |
...
Software must not switch ROM bank while executing interrupt-sensitive code
unless the ISR is guaranteed to exist identically in all banks, or interrupts
are disabled (DI) during the bank switch sequence.
...
; Safe pattern:
    ...
    SETDP $80
    EI              ; re-enable after switch complete
```
