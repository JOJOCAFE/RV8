...
## v1.1: Hardware DI (+1 gate from U33)

### Background
- Current v1.0: DI ($48) is a NOP — IE can only be cleared by /RST or IRQ handler.
- This is safe (ISR always runs with IE=0 after IRQ clears it) but limits flexibility.

### Proposal
- Add DI decode: `DI_decode = T2 & SUB & /XOR & /AC_WR & SRC` → fires on $48.
- Wire DI_decode to U31 IE_FF /CLR (active-low clear).
- **Cost**: Uses spare input on U33 (74HC21 has 2 AND4 gates, one is SETDP, other can be DI).
- **Constraint**: EI already uses one U33 output. Need to verify pin availability.
...
| Hardware DI ($48) | 0 (U33 spare) | LOW | v1.1 |
