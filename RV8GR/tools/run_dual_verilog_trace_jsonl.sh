...
# Settled, observation-only behavioral/chip-level phase trace.
...
mkdir -p "$OUTDIR"
python3 "$ROOT/tools/rv8gr_asm.py" "$ROOT/programs/all_isa_equivalence.asm" \
  -o "$OUTDIR/all_isa_equivalence.memh" -f memh

iverilog -g2012 -Wall -o "$OUTDIR/rv8gr_dual_trace_jsonl.vvp" \
  ...
  "$ROOT/rtl/rv8gr_cpu.v" "$ROOT/rtl/rv8gr_chip_level.v" \
  "$ROOT/tb/tb_rv8gr_dual_trace_jsonl.v"

vvp "$OUTDIR/rv8gr_dual_trace_jsonl.vvp" \
  "+romfile=$OUTDIR/all_isa_equivalence.memh" \
  "+tracefile=$OUTDIR/rv8gr_dual_phase_trace.jsonl" \
  "$@"
