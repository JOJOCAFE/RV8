// RV8GR-V2 chip-level full ROM-program test.
`timescale 1ns/1ps

module tb_rv8gr_v2_chip_full;
  reg clk;
  reg rst_n;
  reg irq_n;
  integer cycle_count;
  ...
  initial begin
    ...
    $dumpfile("rv8gr_v2_chip_full.vcd");
    $dumpvars(0, tb_rv8gr_v2_chip_full);
    ...
    if (pc !== 16'h0066 && pc !== 16'h0067) begin
      $fatal(1, "RV8GR-V2 chip full test timeout: pc=%h phase=%b ir=%02h/%02h ac=%02h pg=%02h z=%b cycles=%0d",
             pc, phase, irh, irl, ac, pg, dut.n_Z_flag, cycle_count);
    end
    if (ac !== 8'h00 || dut.n_Z_flag !== 1'b1 || pg !== 8'h00) begin
      $fatal(1, "RV8GR-V2 chip final state mismatch: pc=%h ac=%02h z=%b pg=%02h cycles=%0d",
             pc, ac, dut.n_Z_flag, pg, cycle_count);
    end

    $display("RV8GR-V2 chip-level full PASS: pc=%h ac=%02h z=%b pg=%02h cycles=%0d",
             pc, ac, dut.n_Z_flag, pg, cycle_count);
    $finish;
  end
endmodule
