...
module tb_rv8gr_opcode_sweep;
    ...
    begin
        ...
        // Page register: PG_CLK = MUX_SEL & ~AC_WR.
        // Physical gate does not block XOR/SUB/JMP/BR reserved encodings.
        if (op[5] && !op[4])
            exp_pg = exp_ibus;
        else
            exp_pg = INIT_PG;

        // Data page: U33 gate 1 = XOR_MODE & /ADDR_MODE & /AC_WR.
        // SUB/MUX/BR/JMP bits are ignored by that physical decode.
        if (op[6] && !(op[3] | op[2]) && !op[4])
            exp_dp = OPERAND;
        else
            exp_dp = INIT_DP;

        // IE: U33 gate 2 = SRC & /XOR_MODE & /AC_WR.
        // DI ($48) is inert because XOR_MODE=1.
        if (op[3] && !op[6] && !op[4])
            exp_ie = 1'b1;
        else
