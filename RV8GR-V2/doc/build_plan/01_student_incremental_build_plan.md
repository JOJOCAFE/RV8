...
This plan follows the v1.0 33-logic-chip wiring guide:
...
- U15/U16 `SEL` temporarily switchable:
  - `SEL=0`: PC low.
  - `SEL=1`: IRL low test value from DIP switches.
- U15/U16 `/E` -> GND.
- LEDs on A0-A7.

**Test**
- With `SEL=0`, A0-A7 follow PC0-PC7.
- With `SEL=1`, A0-A7 follow DIP-switch IRL0-IRL7.
...
- A input = PC8-PC15.
- B input = temporary DP0-DP7 from DIP switches.
- U30-12 is A15.
- LED on A15 at minimum. Better: LEDs on A8-A15.

**Test**
- `ADDR_MODE=0`: A8-A15 follow PC high.
- `ADDR_MODE=1`: A8-A15 follow DIP-switch DP.
...
- U6 CLK <- T1.
- U5 `/OE` -> GND.
- U6 `/OE` temporarily HIGH except when testing operand output.
...
**Remove temporary wiring**
- U6 `/OE` manual test control before connecting `/IRL_OE`.

---

## Stage 9: Basic Control for Fetch and Immediate

**Chips added**: U25 gate 1-2, U26 gate 1-2, U24 gate 6.

**Total packages now**: U24/U25/U26 are added, but some gates remain unused.

**Build**
- U25-1 <- SRC, U25-2 <- STR, U25-3 -> ADDR_MODE.
...
**Total packages now**: 33 logic + ROM + RAM if all previous stages are present.
