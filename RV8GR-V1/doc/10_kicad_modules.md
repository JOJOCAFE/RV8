...
### Global Buses
```
DBUS[7:0]     D0..D7          External data bus (ROM/RAM ↔ U7)
IBUS[7:0]     IB0..IB7        Internal data bus
ABUS[15:0]    A0..A15         Address bus (mux → ROM/RAM)
```
...
BUF_OE_SAFE   U25-8    → IR_BUF (U7 /OE)
...
| DBUS[7:0] | bidir | D0..D7 | ↔ IR_BUF (U7 B-side) |
| A[15:0] | out | A0..A15 | To RV8-Bus, ROM/RAM address |
| A15 | out | A15 | To CTRL (U24-5 for chip select) |

### Wiring
```
=== Address Mux Low: U15-U16 (A0-A7) ===
SEL=0: PC, SEL=1: IRL (for data access)

U15 (A0-A3):
  Pin 1 (SEL) ← ADDR_MODE    Pin 15 (/E) → GND
  Pin 2 (1A) ← PC0    Pin 3 (1B) ← IRL0    Pin 4 (1Y) → A0
  Pin 5 (2A) ← PC1    Pin 6 (2B) ← IRL1    Pin 7 (2Y) → A1
  Pin 11(3A) ← PC2    Pin 10(3B) ← IRL2    Pin 9 (3Y) → A2
  Pin 14(4A) ← PC3    Pin 13(4B) ← IRL3    Pin 12(4Y) → A3

U16 (A4-A7):
  Pin 1 (SEL) ← ADDR_MODE    Pin 15 (/E) → GND
  Pin 2 (1A) ← PC4    Pin 3 (1B) ← IRL4    Pin 4 (1Y) → A4
  Pin 5 (2A) ← PC5    Pin 6 (2B) ← IRL5    Pin 7 (2Y) → A5
  Pin 11(3A) ← PC6    Pin 10(3B) ← IRL6    Pin 9 (3Y) → A6
  Pin 14(4A) ← PC7    Pin 13(4B) ← IRL7    Pin 12(4Y) → A7

=== Address Mux High: U29-U30 (A8-A15) ===
SEL=0: PC high, SEL=1: Data Page

U29 (A8-A11):
  Pin 1 (SEL) ← ADDR_MODE    Pin 15 (/E) → GND
  Pin 2 (1A) ← PC8    Pin 3 (1B) ← DP0    Pin 4 (1Y) → A8
  Pin 5 (2A) ← PC9    Pin 6 (2B) ← DP1    Pin 7 (2Y) → A9
  Pin 11(3A) ← PC10   Pin 10(3B) ← DP2    Pin 9 (3Y) → A10
  Pin 14(4A) ← PC11   Pin 13(4B) ← DP3    Pin 12(4Y) → A11

U30 (A12-A15):
  Pin 1 (SEL) ← ADDR_MODE    Pin 15 (/E) → GND
  Pin 2 (1A) ← PC12   Pin 3 (1B) ← DP4    Pin 4 (1Y) → A12
  Pin 5 (2A) ← PC13   Pin 6 (2B) ← DP5    Pin 7 (2Y) → A13
  Pin 11(3A) ← PC14   Pin 10(3B) ← DP6    Pin 9 (3Y) → A14
  Pin 14(4A) ← PC15   Pin 13(4B) ← DP7    Pin 12(4Y) → A15

=== ROM (AT28C256) ===
  A0-A14 ← A0-A14 (from mux outputs)
  D0-D7  ↔ DBUS
  /CE    ← A15 (direct from U30-12; active when A15=0)
  /OE    → GND
  /WE    → VCC (CPU cannot write ROM; Programmer drives via bus)

=== RAM (62256) ===
  A0-A14 ← A0-A14 (from mux outputs)
  D0-D7  ↔ DBUS
  /CE    ← /A15 (from CTRL, U24-6; active when A15=1)
  /OE    → GND
...
### Chip Select Logic
```
A15=0 → ROM /CE=LOW (ROM active), RAM /CE=HIGH (off)
A15=1 → ROM /CE=HIGH (off), RAM /CE=LOW (RAM active)
...
U7 (Bus Buffer, 74HC245):
  ...
  Pin 19 (/OE) ← BUF_OE_SAFE
  Pin 2-9 (A1-A8) ↔ IB0-IB7 (IBUS side)
...
U14 (AC Output Buffer, 74HC541):
  ...
  Pin 18-11 (Y1-Y8) → IB0-IB7 (IBUS, active only during STORE)
```
...
Hardware guard: BUF_OE_SAFE = BUF_OE_N OR STR (U25 gate 3)
  → If STR=1, U7 disabled regardless of other signals
```
...
| Port | Dir | Net | Connects to |
|------|:---:|-----|-------------|
| IBUS[7:0] | in | IB0..IB7 | From bus (U12/U13 A-inputs) |
...
U12 (bits 0-3):
  Pin 1←IB0,  Pin 2←U19-4,  Pin 3→XOR_Y0
  Pin 4←IB1,  Pin 5←U19-7,  Pin 6→XOR_Y1
  Pin 9←IB2,  Pin 10←U19-9, Pin 8→XOR_Y2
  Pin 12←IB3, Pin 13←U19-12, Pin 11→XOR_Y3

U13 (bits 4-7):
  Pin 1←IB4,  Pin 2←U20-4,  Pin 3→XOR_Y4
  Pin 4←IB5,  Pin 5←U20-7,  Pin 6→XOR_Y5
  Pin 9←IB6,  Pin 10←U20-9, Pin 8→XOR_Y6
  Pin 12←IB7, Pin 13←U20-12, Pin 11→XOR_Y7
...
=== Z Flag: U21 (74HC74, FF1 only) ===
  ...
  Pin 5 (Q1)    → Z_flag
  Pin 6 (/Q1)   → NC
  FF2 unused: Pin 8(/CLR2)→VCC, Pin 9(D2)→GND, Pin 10(CLK2)→GND,
              Pin 11(/PR2)→VCC, Pin 12(Q2)→NC, Pin 13(/Q2)→NC
```
...
| IBUS[7:0] | in | IB0..IB7 | From bus → U23, U32 D-inputs |
...
| BUF_OE_SAFE | out | BUF_OE_SAFE | To IR_BUF (U7 /OE) |
...
Gate 3: Pin 9←BUF_OE_N,  Pin 10←STR,      Pin 8→BUF_OE_SAFE
...
FF2 (IRQ latch):
  Pin 8 (/CLR2) ← /RST
  Pin 9 (D2)    ← VCC
  Pin 10 (CLK2) ← /IRQ (from RV8-Bus)
  Pin 11 (/PR2) ← VCC
  Pin 12 (Q2)   → IRQ_FF (LED)
  Pin 13 (/Q2)  → NC
```
...
3. Use **global labels** for buses: `IB0`..`IB7`, `D0`..`D7`, `A0`..`A15`
