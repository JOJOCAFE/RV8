...
A15=1 → /A15=0 → RAM selected
BUF_OE_SAFE = BUF_OE_N(0) OR STR(0) = 0 → U7 enabled (DBUS→IBUS)
...
## Trace 10: Forbidden Opcode $0C — BUF_OE_SAFE guard
...
*** GUARD ACTIVATES ***
BUF_OE_SAFE = BUF_OE_N(0) OR STR(1) = 1 → U7 DISABLED
U7 /OE = HIGH → U7 tri-stated (cannot drive IBUS)

Result: U14 wins IBUS, U7→DBUS→RAM write proceeds
No bus contention. Store executes. RAM gets AC value.
```
**Result: Hardware guard prevents bus fight ✓**

> 📌 64 opcodes where (opcode & $0C) == $0C are "forbidden" — none are in the ISA.
> Guard ensures deterministic behavior even if executed accidentally.
...
| 7 | LB | $38 | SRC, ADDR_MODE, BUF_OE_SAFE | AC = RAM[{DP,rs}] |
| 8 | SETDP | $40 | XOR_MODE, DP_Load | DP ← operand |
| 9 | BNE | $82 | SUB, BR, Z_match | PC loads if Z=0 |
| 10 | Forbidden | $0C | BUF_OE_SAFE=1 | Store wins, no conflict |
...
| Forbidden (T2) | {DP,IRL} | U14 (AC) — U7 disabled | U7 (write) |
