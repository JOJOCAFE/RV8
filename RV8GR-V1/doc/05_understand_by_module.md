...
2. Main loop ตรวจ IRQ_FF (poll ผ่าน I/O slot)
3. ถ้า IRQ_FF=1 AND IE=1:
   - Software branch ไป handler
   - DI (ปิดรับ interrupt ชั่วคราว)
   - จัดการเสร็จ → EI กลับมารับต่อ
```

> ⚠️ **v1.0: IRQ_FF เคลียร์ได้โดย /RST เท่านั้น!**
> DI ปิดแค่ IE — IRQ_FF ยังค้าง 1 จนกว่าจะ reset
...
**v1.1 (อนาคต, +2 ชิป)**: Hardware vector → PC กระโดดไป $FF00 อัตโนมัติ
